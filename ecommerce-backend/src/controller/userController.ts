...
export const newUser = TryCatch(
  async (
    ...
    if (!name || !email || !photo || !_id || !dob || !gender) {
      const requiredFields: (keyof INewUserRequestBody)[] = [
        "name",
        "email",
        "photo",
        "_id",
...
export const bulkUserEntry = TryCatch(async (req, res, next) => {
  ...
  let missingFieldUsers = users.map((user, index) => {
    ...
    if (!photo) missingField.push("Photo");
    if (!_id) missingField.push("ID");
    if (!dob) missingField.push("Date of birth");
    if (!gender) missingField.push("Gender");

    return { index, missingField };
  });

  missingFieldUsers = missingFieldUsers.filter(
    (user) => user.missingField.length > 0
  );
  ...
  if (missingFieldUsers.length > 0) {
    const errorMessageArray = missingFieldUsers.map((user) => {
      return `User at index ${
        user.index
      } : missing fields : ${user.missingField.join(", ")}`;
    });

    const errorMessage = errorMessageArray.join(", ");

    return next(
      new ErrorHandler(`Please enter all fields ${errorMessage}`, 400)
    );
  }
  ...
  if (existingUsers.length) {
    ...
    return next(
      new ErrorHandler(`Emails already exists: ${existingEmails}`, 400)
    );
  }
